...
Đã có [evidence packet boundary draft](evidence-packet.md) với provenance, model-input/evaluator separation và public citation mock. Packet integrity, evidence sufficiency và current access là ba kiểm tra độc lập; ví dụ hợp lệ không chứng minh source được serving hoặc model đã chạy.

## Query input

- Request/conversation identity và câu hỏi, mode mong muốn.
- Principal/scope được backend xác thực: course/material access, quyền hoạt động và policy version. Không lấy role từ prompt hay payload tự khai.
- Corpus/index snapshot, ngôn ngữ, context và time/token budget; chỉ truyền lịch sử cần thiết, đã được phép.

AI core vẫn phải áp scope ở retrieval và fetch nguồn/parent/image, không chỉ ở generation. Thiếu scope thì fail closed. In-process boundary không tự tạo sandbox: cấu hình credentials/tools vẫn phải least privilege.
...
Backend sở hữu material lifecycle, review approval và job identity; AI core sở hữu parse/chunk/index algorithms và index version. Job chỉ được phép xử lý theo use scope đã duyệt, không biến research-only thành production-ready.
